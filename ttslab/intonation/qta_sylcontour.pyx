# -*- coding: utf-8 -*-
""" Implementation of the "Quantitative Target Approximation"
    synthesis formula for a syllable: 
...
def sylcontour(np.ndarray[DTYPE_FLOAT_t, ndim=1] t,
               float m,
               float b,
               float p0,
               float dp0,
               ...
               float l):
    cdef float c1, c2, c3
    #cdef np.ndarray[DTYPE_FLOAT_t, ndim=1] pt
    cdef np.ndarray[DTYPE_FLOAT_t, ndim=1] xt = m*t + b
    # pl.plot(t, xt, linestyle="dashed", color="red")
    c1 = p0 - b
    c2 = dp0 + c1 * l - m
    c3 = (ddp0 + 2 * c2 * l - c1 * l ** 2) / 2.0
    cdef np.ndarray[DTYPE_FLOAT_t, ndim=1] pt = xt + (c1 + c2*t + c3*t**2) * np.exp(-l * t)
    return pt
