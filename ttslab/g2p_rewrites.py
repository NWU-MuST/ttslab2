...
if __name__ == "__main__":
    ...
    g2p = ttslab.g2p_rewrites.G2P_Rewrites_Semicolon()
    g2p.load_ruleset_semicolon(args.rulesfn)
    if args.gnullsfn:
        g2p.load_gnulls(args.gnullsfn)
    if args.pmapfn:
        g2p.load_simple_phonemapfile(args.pmapfn)
        g2p.map_phones()
    if args.gmapfn:
        g2p.load_simple_graphmapfile(args.gmapfn)
        g2p.map_graphs()
